...
sub GetSessionData{
  my ($dbh,$key)=@_;
  my $sth=$dbh->prepare("select * from session inner join account on session.id = account.id where session.key = '".escape_sql($key)."';");
  $sth->execute;
...
sub GetAccountFromId{
  my ($dbh,$id)=@_;
  my $id=ParseInt($id);
  my $sth=$dbh->prepare("select * from account where id = $id");
  $sth->execute;
  return $sth->fetchrow_hashref();
}
